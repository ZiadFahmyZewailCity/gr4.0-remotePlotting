#ifndef GNURADIO_DASHBOARDBLOCKS_IMGUI_CHECKBOX_HPP
#define GNURADIO_DASHBOARDBLOCKS_IMGUI_CHECKBOX_HPP
...
namespace gr::dashboard_blocks {
    ...
    template <typename T>
    struct imGUI_checkBox : gr::Block<imGUI_checkBox<T>> {

        using Description = gr::Doc<R"(
        Name: checkBox
        Block Type: Widget
        What it does: A checkbox toggle displayed on the dashboard, bound to a boolean flowgraph variable.

        API:
        - id (string): unique widget identifier, must be unique per instance
        - panel (string): panel this widget is grouped under
        - title (string): caption shown next to the widget
        - target_property (string): name of the flowgraph property this widget targets
        - on_val_update(bool): callback fired when the dashboard sends a new value
        - get_external_val() -> bool: callback polled each cycle to pull the flowgraph's value
        )">;
        ...
        imGUI_checkBox(gr::property_map initial_settings = {})
            : gr::Block<imGUI_checkBox<T>>(initial_settings)
        {
        ...
        GR_MAKE_REFLECTABLE(imGUI_checkBox, out, panel_name, widget_id, target_property, endpoint, dashboard_server, current_val, title);
...
GR_REGISTER_BLOCK("gr::dashboard_blocks::imGUI_checkBox", gr::dashboard_blocks::imGUI_checkBox, [bool])
